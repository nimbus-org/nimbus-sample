...
○目標
 コネクションをグルーピングして、１対多でのメッセージ配信をしてみよう。
...
○実行
  ...
  （２）任意のサブジェクトにメッセージを送信するMainクラスを実行する。
      例：
  ...
  （４）サブジェクト"fuga"のメッセージを受信するクライアント２を起動する。
      例：
        java -classpath .;nimbus.jar;jmx.jar -DMode=Client -DSubject=fuga jp.ossc.nimbus.core.ServiceManagerFactory -server service-definition.xml
